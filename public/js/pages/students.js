...
$(document).ready(function () {
    ...
    function refresh_student_info(id) {
        ...
        $('#student_soa_table').DataTable({
            ...
            columns: [{ data: 'description.name' }, { data: 'amount_due' }, { data: 'amount_paid' }, { data: 'payment_date' }, { data: 'balance' }, { data: 'verified.fname' }, { data: 'remarks' }],
            columnDefs: [{ defaultContent: "", targets: "_all" }, {
    ...
    $(document).on('click', '.o_switch', function () {
        var o_switch = $('.o_switch');
        var o_index = o_switch.index($(this));
    ...
    $(document).on('click', '.o_delete', function () {
        var o_delete = $('.o_delete');
        var o_index = o_delete.index($(this));
        ...
        $.ajax({
            ...
            success: function success(data) {
                ...
                if (data.soa.length > 6) {
                    for (var _x = 0; _x < data.soa.length - 6; _x++) {
                        generate_others(data.soa[_x + 6], data.soa_fees);
                    }
                }
                compute_balance();
                $('#student_info_modal').modal('hide');
    ...
    $(document).on('click', '.delete_emergency', function () {
        var id = $(this).attr('id');
        var text = 'You are about to delete an emergency contact.';
    ...
    $(document).on('click', '.delete_emp_history', function () {
        var id = $(this).attr('id');
        var text = 'You are about to delete an employment history.';
    ...
    $(document).on('click', '.delete_education', function () {
        var id = $(this).attr('id');
        var text = 'You are about to delete an education background.';
    ...
    $(document).on('click', '.delete_soa', function () {
        var id = $(this).attr('id');
        var text = 'You are about to delete a soa.';
    ...
    function generate_others(data, soa_fees) {
        ...
        if (data) {
            for (var _x2 = 0; _x2 < soa_fees.length; _x2++) {
                desc_html += '<option value="' + soa_fees[_x2].id + '">' + soa_fees[_x2].name + '</option>';
            }
    ...
    function o_desc_select() {
        for (var x = 0; x < $('.o_desc_select').length; x++) {
            $('.soa_o_verified').eq(x).select2();
            var o_desc = $('.o_desc_select').eq(x).next('.select2-container');
            if (o_desc.length == 0) {
